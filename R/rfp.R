...
#############################################################################
rfp <- function(mymodel, id=c("all","none"), ...){
standardGeneric("rfp")
}
setGeneric("rfp", def=rfp)
#############################################################################
###                                                                   synonym
#############################################################################
Residuals.Fitted.CategoricalFixedEffects <- rfp
#############################################################################
###                                                                      LMER
#############################################################################
rfp.lmer <- function(mymodel, id=c("all", "none"), ...){
### ================================= checking
  id <- match.arg(id)
### ================================= reading
  ...
  #
  this.data <- cbind(my.data,fi)
### ================================= makint the panels
  for ( i in seq(along=fixed.terms.factor)){
    ## i is a number for every random factor
    plot.eval <- paste("plot_",i,"<-xyplot(ri ~ fi|",fixed.terms.factor[i]," , this.data,
                     xlab=\"fitted values\",
                     ylab=\"residuals\",
...
### ================================= panel arangement
  ...
  grid.text("rfp.lmer(), residuals vs. fitted for all categorical fixed effects", y=unit(0.975, "npc"), just=c("center","bottom"), gp=gpar(col="grey", fontface="bold", fontsize=16 ))
